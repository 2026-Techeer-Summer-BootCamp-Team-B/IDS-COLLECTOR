...
> 이 문서는 루트 `README.md`가 서비스 소개 위주로 재구성되면서(2026-07-17) 옮겨온
> 예전 README 원본입니다. 데이터 파이프라인 상세 흐름, 정규화 계약, API 명세,
> 저장소별 스키마, 트러블슈팅 기록 등 백엔드 구현 디테일은 여기서 계속 관리합니다.
> 서비스 전체 소개/스크린샷/아키텍처 요약은 루트 `README.md`를 먼저 보세요.
...
- 현재 32개 시나리오(S1~S32, S26~S32 추가분 포함) - S10/S19/S20/S21/S22/S23을
  제외한 나머지는 falcosecurity/plugins의 실제 K8s audit 룰에 근거한 설계, 엔진
  검증용 예시가 아님(`app/scenarios/README.md` 참고).
- **S26~S30**: WAF/WAS 신호만으로 발화하는 시나리오 5개 - 로그인 브루트포스(T1110),
  Rate Limit 남용(T1499), 스캐너 User-Agent(T1046), JWT 위조(T1550), 404 다발
  탐색(T1046).
- **S31**: K8s RBAC 권한/역할 열거 정황(T1069, `discovery.yaml`, k8s_audit 기반).
- **S32**: 컨테이너 내 리버스 쉘/원격 코드 실행 의심(T1059, `workload.yaml`,
  falcosecurity/rules 코어 "Redirect STDOUT/STDIN to Network Connection in
  Container" 룰 기반, 실 클러스터 로그로 발화 확인).
