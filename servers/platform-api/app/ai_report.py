...
없었음) Gemini를 다시 호출하지 않고 ai_trend_report_cache(016 마이그레이션)에
...
async def _get_cached_message(days: int, stats_hash: str) -> Optional[str]:
    async with pool().acquire() as conn:
        row = await conn.fetchrow(
            "SELECT message FROM ai_trend_report_cache WHERE days = $1 AND stats_hash = $2",
            days,
            stats_hash,
        )
...
async def _save_cache(days: int, stats_hash: str, message: str) -> None:
    async with pool().acquire() as conn:
        await conn.execute(
            """
            ...
            SET stats_hash = EXCLUDED.stats_hash, message = EXCLUDED.message, generated_at = now()
            """,
            days,
            stats_hash,
            message,
...
async def generate_trend_report(days: int = 7) -> Dict[str, Any]:
    ...
    cached_message = await _get_cached_message(days, stats_hash)
    ...
    await _save_cache(days, stats_hash, response.text)
