...
- 인증 (P5-2): app/auth.py - login/logout/session, users 테이블(pgcrypto 해시) 실사용자 검증.
  ...
  포함)는 로그인만, 쓰기(POST/PATCH/DELETE)는 role=admin만 통과. WebSocket은 브라우저가
  커스텀 헤더를 못 보내서 `?token=` 쿼리스트링으로 토큰을 받아 /verify가 X-Forwarded-Uri에서
  파싱한다. 세션은 아직 메모리 토큰 스토어(재시작 시 무효화), 영속화는 팀 설계 후 채울 것.
  주의: platform-api:8400 직결 포트(로컬 디버깅용)는 Traefik을 안 거치므로 이 인증이
  전혀 적용되지 않는다
