...
class EventIncidentOut(BaseModel):
...
@router.get("/{event_id}/incident", response_model=EventIncidentOut)
async def get_event_incident(event_id: str):
    """이 event_id가 correlation-engine에 의해 어느 인시던트로 묶였는지 정확히
    조회한다(2026-07-17, CRITICAL 토스트의 "공격 스토리라인 보기" 버튼이 씀).
    correlation_key로 후보를 추린 뒤 검증하던 기존 프론트 우회 로직을 대체 -
    idx_incident_events_event_id 인덱스(datastore/postgres/init/024-...) 덕에
    단일 인덱스 조회로 끝난다. 아직 어떤 인시던트에도 안 묶였으면(=correlation-engine이
    아직 처리 전) incident_id: null을 반환 - 404가 아니라 200으로, "아직 없음"은
    폴링 중인 정상 상태지 에러가 아니라서다."""
    async with pool().acquire() as conn:
        row = await conn.fetchrow(
            "SELECT incident_id FROM incident_events WHERE event_id = $1 LIMIT 1",
            event_id,
        )
    return EventIncidentOut(incident_id=str(row["incident_id"]) if row else None)
