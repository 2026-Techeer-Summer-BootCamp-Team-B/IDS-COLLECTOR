"""AlertConfig API (/alert-configs) - Slack/Discord 알림 설정 CRUD (스켈레톤).
app/notifications.py가 이 테이블을 읽어서 실제 발송 여부/대상을 결정하도록
바꾸는 작업은 아직 안 함 - 지금은 CRUD만."""
...
async def create_alert_config(body: AlertConfigIn):
    async with pool().acquire() as conn:
...
async def update_alert_config(config_id: str, body: AlertConfigIn):
    async with pool().acquire() as conn:
        row = await conn.fetchrow(
            """
